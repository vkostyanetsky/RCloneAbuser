...
Скрипт, который последовательно запускает утилиту [rclone](https://rclone.org) для списка файлов и папок, который ему предоставлен. Я использую его для бэкапа на домашний NAS: это несколько удобнее, чем просто набор длиннющих одинаковых команд в нескольких cmd-файлах.
